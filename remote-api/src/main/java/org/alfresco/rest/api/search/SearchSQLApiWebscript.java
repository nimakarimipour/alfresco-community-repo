                                                                        ...
                                                                        InitializingBean
{
    ...
    public void execute(WebScriptRequest webScriptRequest, WebScriptResponse res) throws IOException
    {
        try
        {
            ...
            @RUntainted SearchParameters sparams = buildSearchParameters(searchQuery);

            @RUntainted ResultSet results = searchService.query(sparams);
            @RUntainted FilteringResultSet frs = (FilteringResultSet) results;
            @RUntainted SolrSQLJSONResultSet ssjr = (SolrSQLJSONResultSet) frs.getUnFilteredResultSet();
